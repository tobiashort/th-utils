...
type Args struct {
	Head  int    `clap:"short=H,desc='Number of beginning lines to skip'"`
	Tail  int    `clap:"desc='Number of ending lines to skip'"`
	Lines string `clap:"desc='Specific lines to skip, e.g. 2,3,5-10,24'"`
	File  string `clap:"positional,desc='The file to read. Reads from Stdin if not specified.'"`
}

func linesToSkip(head int, lines string, tail int, maxLines int) []int {
	linesToSkip := []int{}
...
func run(args Args) {
	...
	linesToSkip := linesToSkip(args.Head, args.Lines, args.Tail, len(lines))

	for i, l := range lines {
		if len(linesToSkip) > 0 {
			if i == linesToSkip[0]-1 {
